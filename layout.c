...
void Initialize_Map(Rectangle (*map)[NUM_OF_RECTANGLES_Y][NUM_OF_RECTANGLES_X])
{
    ...
    for (int i = 0; i < NUM_OF_RECTANGLES_X; i++)
        ...
        {
            float x_value = (GetScreenWidth() * ((float)i + 1) / (NUM_OF_RECTANGLES_X + 1)) - SIZE_OF_RECTANGLES_X / 2.0f;
            float y_value = (GetScreenHeight() * ((float)j + 1) / (NUM_OF_RECTANGLES_Y + 1)) - SIZE_OF_RECTANGLES_Y / 2.0f;
